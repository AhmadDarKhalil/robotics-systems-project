...
int mm2count(float mm) // covert mm to encoder counts
{
  float result;
  result = mm*ENCODER_COUNT_PER_REV/(2 * WHEEL_RADIUS_MM * 3.14);
  return (int) result;
}
int count2mm(float count) // covert  encoder counts to mm
{
  float result;
  result = count *(2 * WHEEL_RADIUS_MM * 3.14)/ENCODER_COUNT_PER_REV;
  return (int) result;
}
void move_mm(float mm, Motor leftMotor, Motor rightMotor , int speed)
{
  setupEncoder0();
  setupEncoder1();
  // move for mm distance
  while (abs(count_e0) < mm2count (mm) || abs(count_e1) < mm2count (mm)){
    
    if (abs(count_e0) < mm2count (mm)) // move for mm millimiters
    leftMotor.move(speed);
  else
      leftMotor.move(0);

  if (abs(count_e1) < mm2count (mm))
...
void rotate (float degree, Motor leftMotor, Motor rightMotor, int direction) // direction: 1=> clockwise, -1 =>counterclockwise
{
  setupEncoder0();
  setupEncoder1();
  float counts = degree / 180 * 1500 ; // this is the ratio to get the required counts for any degree
  // move for mm distance
  while (abs(count_e0) < counts || abs(count_e1) < counts){
    
    if (abs(count_e0) < counts) // move for mm millimiters
    leftMotor.move(direction * POWER_FACTOR_FOR_ENCOERS);
  else
      leftMotor.move(0);

  if (abs(count_e1) < counts)
      rightMotor.move(-direction * POWER_FACTOR_FOR_ENCOERS);
  else
      rightMotor.move(0);
